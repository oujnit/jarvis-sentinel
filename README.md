...
- 🌡️ **SYSTEM & QUOTA 遥测卡**（数据源参照 [kindle-ai-quota-dashboard](https://github.com/softmutiny/kindle-ai-quota-dashboard)）：
  ...
  - **DeepSeek / GLM 余额与额度窗口进度条**：GLM 每个额度窗口显示已用百分比进度条（≥70% 金色、≥90% 红色报警）、剩余 `—` 与重置倒计时；Claude（读 `~/.claude/.credentials.json`）与 Codex（调本机 `codex app-server`）的额度窗口同样尝试采集，未配置则省略。凭证自动读本机（见下文）
